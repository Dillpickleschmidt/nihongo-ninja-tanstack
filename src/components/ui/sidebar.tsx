...
const Sidebar: Component<SidebarProps> = (rawProps) => {
  ...
  return (
    <Switch>
      ...
      <Match when={isMobile()}>
        {/* Backdrop overlay */}
        <Show when={openMobile()}>
          <div
            class="fixed inset-0 z-10 bg-black/50 transition-opacity"
            onClick={() => setOpenMobile(false)}
          />
        </Show>
        {/* Same fixed sidebar as desktop, slides in/out */}
        <div
          class={cn(
            "fixed inset-y-0 z-20 flex h-svh w-(--sidebar-width) flex-col transition-[left,right] duration-200 ease-linear",
            local.side === "left"
              ? openMobile()
                ? "left-0"
          ...
          {...others}
        >
          <div class="bg-sidebar flex size-full flex-col">{local.children}</div>
        </div>
      </Match>
      <Match when={!isMobile()}>
        <div
