...
export default new class MALSync {
  auth = createPersisted<MALOAuth | undefined>('malAuth', undefined)
  viewer = createPersisted<ResultOf<typeof UserFrag> | undefined>('malViewer', undefined)
  ...
  async _post<T>(url: string, body?: Record<string, unknown>): Promise<T | { error: string }> {
    return await this._request<T>(url, 'POST', new URLSearchParams(body as Record<string, string>))
  }

  async _patch<T>(url: string, body: Record<string, unknown>): Promise<T | { error: string }> {
    return await this._request<T>(url, 'PATCH', new URLSearchParams(body as Record<string, string>))
  }
  ...
  async _refresh() {
    ...
    const data = await this._post<MALOAuth>(ENDPOINTS.API_OAUTH, {
      client_id: malClientID.value,
  ...
  async login() {
    ...
    const clientID = malClientID.value
    const state = crypto.randomUUID().replaceAll('-', '')
    const challenge = (crypto.randomUUID() + crypto.randomUUID()).replaceAll('-', '')

    const redirect = import.meta.env.DEV ? 'http://localhost:7344/authorize' : 'https://hayase.app/authorize'

    const { code } = await native.authMAL(
      `${ENDPOINTS.API_AUTHORIZE}?response_type=code&client_id=${clientID}&state=${state}&code_challenge=${challenge}&code_challenge_method=plain&redirect_uri=${redirect}`
    )
  ...
  async entry(variables: VariablesOf<typeof Entry>) {
    ...
    if (!malId) {
      console.error('Could not find MAL ID for this media.')
      return
    }
