...
export default new class KitsuSync {
  auth = createPersisted<OAuth | undefined>('kitsuAuth', undefined)
  viewer = createPersisted<ResultOf<typeof UserFrag> | undefined>('kitsuViewer', undefined)
  userlist = { value: {} as Record<string, ResultOf<typeof FullMediaList>> }
  favorites = { value: {} as Record<string, string> }
  kitsuToAL: Record<string, string> = {}
  ALToKitsu: Record<string, string> = {}

  private userlistSubscribers = new Set<(val: Record<string, ResultOf<typeof FullMediaList>>) => void>()
  private favoritesSubscribers = new Set<(val: Record<string, string>) => void>()

  continueIDs = createMemo(() => {
    const entries = Object.entries(this.userlist.value)
  ...
  async _request<T = object>(url: string | URL, method: string, body?: unknown): Promise<T | KitsuError> {
    const auth = this.auth.value
    try {
      if (auth) {
  ...
  async _refresh() {
    ...
    const auth = this.auth.value
    const data = await this._post<OAuth>(ENDPOINTS.API_OAUTH, {
  ...
  async login(username: string, password: string) {
    ...
    const data = await this._request<OAuth>(ENDPOINTS.API_OAUTH, 'POST', {
      grant_type: 'password',
      username,
      password
    })
  ...
  async _user() {
    ...
    const res = await this._get<Res<User>>(ENDPOINTS.API_USER_FETCH, {
      'filter[self]': true,
  ...
  _kitsuEntryToAl(entry: Resource<KEntry>): ResultOf<typeof FullMediaList> {
    return {
      id: Number(entry.id),
      status: entry.attributes.reconsuming
        ? 'REPEATING'
        : entry.attributes.status
          ? KITSU_TO_AL_STATUS[entry.attributes.status]
          : null,
  ...
  _entriesToML(res: Res<KEntry | User, Anime | Mapping | KEntry | Fav>) {
    ...
    for (const entry of entries) {
      const animeRes = Array.isArray(entry.relationships?.anime?.data)
        ? entry.relationships.anime.data[0]
        : entry.relationships?.anime?.data
      const anime = relations.anime.get(animeRes?.id ?? '')
      const ids = Array.isArray(anime?.relationships?.mappings?.data)
        ? anime.relationships.mappings.data
        : [anime?.relationships?.mappings?.data]
      const anilistId = ids
        .map((i) => (i ? relations.mappings.get(i.id) : undefined))
        .filter((i) => i)[0]?.attributes.externalId
  ...
  async _makeFavourite(kitsuAnimeId: string) {
    ...
    const data = await this._post<ResSingle<Fav>>(ENDPOINTS.API_FAVOURITES, {
      data: {
        relationships: {
    ...
    this.notifyFavoritesSubscribers()
  }
  ...
  async _addEntry(id: string, attributes: Omit<KEntry, 'createdAt' | 'updatedAt'>, alId: number) {
    ...
    const viewer = this.viewer.value
    const data = await this._post<ResSingle<KEntry>>(ENDPOINTS.API_USER_LIBRARY, {
      data: {
        attributes,
        relationships: {
    ...
    this.notifyUserlistSubscribers()
  }
  ...
  async _updateEntry(id: number, attributes: Omit<KEntry, 'createdAt' | 'updatedAt'>, alId: number) {
    ...
    const data = await this._patch<ResSingle<KEntry>>(`${ENDPOINTS.API_USER_LIBRARY}/${id}`, {
  ...
  hasAuth = createMemo(() => {
    return this.viewer.value !== undefined && !!this.viewer.value?.id
  })

  id() {
    return this.viewer.value?.id
  }
  ...
  async toggleFav(id: number) {
    ...
    if (!kitsuId) {
      console.error('Could not find Kitsu ID for this media.')
      return
    }
  ...
  private notifyUserlistSubscribers() {
    this.userlistSubscribers.forEach((cb) => cb(this.userlist.value))
  }

  private notifyFavoritesSubscribers() {
    this.favoritesSubscribers.forEach((cb) => cb(this.favorites.value))
  }

  subscribeUserlist(cb: (val: Record<string, ResultOf<typeof FullMediaList>>) => void) {
    this.userlistSubscribers.add(cb)
    cb(this.userlist.value)
    return () => this.userlistSubscribers.delete(cb)
  }

  subscribeFavorites(cb: (val: Record<string, string>) => void) {
    this.favoritesSubscribers.add(cb)
    cb(this.favorites.value)
    return () => this.favoritesSubscribers.delete(cb)
  }
}()
