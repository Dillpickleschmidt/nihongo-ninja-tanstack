...
export class KanaToKanjiOverlay {
  constructor(private answerChecker: AnswerChecker) {}
   ...
   * @param rawQuestion - Raw question from Convex (for extracting kanji from furigana)
   */
  overlayKanji(
    userInput: string,
    question: PracticeQuestion,
    rawQuestion: Doc<"sentencePracticeQuestions">,
  ): OverlayResult | null {
    // Skip if input contains kanji
    if (textProcessor.containsKanji(userInput)) {
      return null
    }
    ...
    const parts = rawSegments.flatMap((segment: SentenceSegment) => {
