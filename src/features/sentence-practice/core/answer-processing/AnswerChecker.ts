...
export class AnswerChecker {
  ...
  checkAnswer(input: string, question: PracticeQuestion): CheckResult {
    let userText = textProcessor.normalize(input)
    const shouldUseKana = !textProcessor.containsKanji(input)

    const answerTexts = question.answers.map((answer) =>
      textProcessor.extractPlainText(answer.segments),
    )
    ...
    const matches = question.answers
      .map((answer) => {
        ...
        const correctText = textProcessor.extractPlainText(answer.segments)
        const textForComparison = shouldUseKana
          ? textProcessor.convertToKana(correctText)
          : correctText
