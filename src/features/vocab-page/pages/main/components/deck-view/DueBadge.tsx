...
export function DueBadge(props: DueBadgeProps) {
  ...
  return (
    <Show
      ...
      fallback={
        <span class={`inline-flex h-5 w-10 rounded-full ${loadingColor()}`} />
      }
    >
      <Show when={props.isDue}>
        <div class="ml-2 flex flex-col items-end gap-1">
          <span
            class={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-semibold uppercase ${colors()}`}
          >
            Due
          </span>
