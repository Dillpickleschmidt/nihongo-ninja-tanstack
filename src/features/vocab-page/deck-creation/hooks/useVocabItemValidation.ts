...
import { VocabItemFieldValidationSchema } from "../schemas/vocab-item-schemas"
import type { VocabItemValidationContext, FieldValidationState } from "../types/deck-creation-types"

export function useVocabItemValidation(context: () => VocabItemValidationContext) {
  // Validate word field
  ...
  const englishValidation = createMemo((): FieldValidationState => {
    ...
    const hasValidMeaning = englishMeanings.some(meaning => meaning.trim().length > 0)
    const isRequired = true
    
    return {
      isValid: hasValidMeaning,
      error: !hasValidMeaning ? "At least one English meaning is required" : undefined,
      isRequired,
      showError: false // Let component decide when to show
    }
  })
