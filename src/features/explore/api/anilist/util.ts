...
export function getParentForSpecial(media: Media) {
  if (!["SPECIAL", "OVA", "ONA"].some((format) => media.format === format))
    return false
  const animeRelations = (media.relations?.edges?.filter(
    (edge) => edge?.node?.type === "ANIME",
  ) ?? []) as MediaEdge[]

  return (
    getRelation(animeRelations, "PARENT") ??
    getRelation(animeRelations, "PREQUEL") ??
    getRelation(animeRelations, "SEQUEL")
  )
}

export function getRelation(
  list: MediaEdge[],
  type: MediaEdge["relationType"],
) {
...
export function episodes(
  media: Pick<
    Media,
    "aired" | "notaired" | "episodes" | "mediaListEntry" | "id"
  >,
): number | undefined {
...
export function season(media: Pick<Media, "season" | "seasonYear">) {
  return [media.season?.toLowerCase(), media.seasonYear]
    .filter((s) => s)
    .join(" ")
}
...
export function isMovie(
  media: Pick<Media, "format" | "title" | "synonyms" | "duration" | "episodes">,
) {
  if (media.format === "MOVIE") return true
  if (
    [...Object.values(media.title ?? {}), ...(media.synonyms ?? [])].some(
      (title) => title?.toLowerCase().includes("movie"),
    )
  )
    return true
...
export function dedupeAiring(media: ResultOf<typeof ScheduleMedia>) {
  return [...(media.aired?.n ?? []), ...(media.notaired?.n ?? [])].filter(
    (v, i, a) => v != null && a.findIndex((s) => s?.e === v.e) === i,
