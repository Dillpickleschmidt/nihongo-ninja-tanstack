...
export function CreateSubtitlePathDialog(props: {
  open: boolean
  onClose: () => void
}) {
  ...
  const handleFileSelect = async (file: File) => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to process subtitle file",
      )
    } finally {
  ...
  const handleSave = async () => {
    ...
    try {
      const { selectedGrammarModules, selectedVocabDecks } =
      ...
      setError(
        err instanceof Error ? err.message : "Failed to save learning path",
      )
    } finally {
      setIsProcessing(false)
    }
    ...
    >
      <DialogPrimitive.Portal>
        ...
        <DialogPrimitive.Content
          class="fixed top-1/2 left-1/2 z-50 max-h-[90vh] w-full max-w-2xl -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-2xl border border-white/10 bg-neutral-950 p-6 data-[expanded]:animate-in data-[closed]:animate-out data-[closed]:fade-out-0 data-[expanded]:fade-in-0"
          onCloseAutoFocus={(e) => e.preventDefault()}
        >
          <DialogPrimitive.Title class="text-xl font-semibold text-white">
            Create from Subtitles
          </DialogPrimitive.Title>
          <DialogPrimitive.Description class="mt-1 text-sm text-white/50">
            Upload an .srt subtitle file to generate a custom learning path
          </DialogPrimitive.Description>

          <DialogPrimitive.CloseButton class="absolute top-4 right-4 cursor-pointer rounded-full p-1.5 text-white/50 transition-colors hover:bg-white/10 hover:text-white">
            <X class="size-4" />
              ...
              <div class="mt-6 space-y-4 rounded-xl border border-white/10 bg-white/5 p-4">
                <div class="text-sm text-white/80">
                  ...
                  <p>
                    ...
                    {data().grammarPatterns.length}
                  </p>
                  <p>
                    Generated modules:{" "}
                    {data().modules.filter((m) => m.type === "grammar").length}{" "}
                    grammar,{" "}
                    {
                      data().modules.filter((m) => m.type === "vocabulary")
                        .length
                    }{" "}
                    vocab decks
                  </p>
                ...
                <div>
                  ...
                  <ul class="space-y-1 text-sm text-white/70">
                    <For
                      each={data().modules.filter((m) => m.type === "grammar")}
                    >
                  ...
                  <ul class="space-y-2 text-sm text-white/70">
                    ...
                    >
                      {(deck) => (
                        <li>
                          <p>
                            {deck.isVerbDeck ? "Verbs" : "Non-verbs"} -{" "}
                            {deck.words.length} items
                          </p>
