...
const MNEMONIC_REPLACEMENTS = [
  { pattern: /<radical>([^<]+)<\/radical>/g, replacement: '<span class="text-purple-500 font-medium">$1</span>' },
  { pattern: /<kanji>([^<]+)<\/kanji>/g, replacement: '<span class="text-indigo-500 font-medium">$1</span>' },
  { pattern: /<reading>([^<]+)<\/reading>/g, replacement: '<span class="text-orange-500 font-medium">$1</span>' },
] as const
