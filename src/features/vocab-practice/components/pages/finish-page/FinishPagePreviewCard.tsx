// features/vocab-practice/components/pages/finish-page/FinishPagePreviewCard.tsx
...
type FinishPagePreviewCardProps = {
  card: PracticeCard
  index: number
  allCards: PracticeCard[]
  incorrectCount: number
  vocabularyDependencies: string[]
}

export default function FinishPagePreviewCard(
  props: FinishPagePreviewCardProps,
) {
  ...
  const RightColumn = () => (
    <div class="flex h-full min-w-[90px] flex-col items-end justify-between">
      {/* Top row: Miss count + number */}
      <div class="flex items-center gap-2">
        <Show when={props.incorrectCount > 0}>
          <span class="inline-flex items-center rounded-full bg-red-500/20 px-2 py-0.5 text-xs font-semibold text-red-500 uppercase">
            {props.incorrectCount} miss{props.incorrectCount > 1 ? "es" : ""}
          </span>
        </Show>
            ...
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                >
                  ...
                  <Show when={props.vocabularyDependencies.length > 0}>
                    <span class="text-muted-foreground/80 ml-2 text-xs">
                      dependency of {props.vocabularyDependencies.join(", ")}
                    </span>
                  </Show>
                </p>
              </div>
