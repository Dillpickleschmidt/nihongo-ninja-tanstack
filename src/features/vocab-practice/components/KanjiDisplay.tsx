...
import { KanjiAnimation, type KanjiDisplaySettings, type KanjiAnimationSettings, type KanjiStyleSettings } from '@/components/KanjiAnimation'
...
export function KanjiDisplay(props: Props) {
  ...
  const [displaySettings, setDisplaySettings] = createStore<KanjiDisplaySettings>({ ...DEFAULT_DISPLAY_SETTINGS })
  const [animationSettings, setAnimationSettings] = createStore<KanjiAnimationSettings>({ ...DEFAULT_ANIMATION_SETTINGS })
          ...
          >
            {(animationRef) => (
              <KanjiAnimationControls
                ...
                displaySettings={displaySettings}
                animationSettings={animationSettings}
                onDisplaySettingsChange={(settings) => setDisplaySettings(settings)}
                onAnimationSettingsChange={(settings) => setAnimationSettings(settings)}
                processedSvgContent={processSvgString(svgData()!, {
                  size: styleSettings.size,
                  strokeColor: styleSettings.strokeColor,
                  strokeWidth: styleSettings.strokeWidth,
