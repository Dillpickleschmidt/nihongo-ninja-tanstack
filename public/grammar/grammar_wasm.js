 ...
 * @returns {any}
 */
export function analyze_batch(token_arrays_js) {
    const ret = wasm.analyze_batch(token_arrays_js);
    if (ret[2]) {
 ...
 * @returns {any}
 */
export function analyze_single(tokens_js) {
    const ret = wasm.analyze_single(tokens_js);
    if (ret[2]) {
        throw takeFromExternrefTable0(ret[1]);
    }
    return takeFromExternrefTable0(ret[0]);
