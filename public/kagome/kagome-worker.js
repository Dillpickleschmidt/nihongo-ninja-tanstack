...
// Custom Go WASM Runtime Class (from extension, adapted for worker)
class Go {
  constructor() {
    this.argv = ["js"]
    this.env = {}
...
async function loadKagomeWasm() {
  ...
  loadPromise = (async () => {
    try {
      ...
      return new Promise((resolve, reject) => {
        const checkInterval = setInterval(() => {
          if (typeof self.kagome_tokenize === "function") {
            ...
            reject(
              new Error(
                "Kagome WASM failed to initialize - tokenize function not available",
              ),
            )
          }
          attempts++
      ...
      console.error("[Kagome Worker] Failed to load Kagome WASM:", error)
      throw error
    }
  })()
...
Promise.all([loadKagomeWasm(), loadGrammarWasm()])
  ...
  .catch((error) => {
    console.error("[Kagome Worker] Initialization failed:", error)
    self.postMessage({
      type: "error",
      message: error.message,
    })
