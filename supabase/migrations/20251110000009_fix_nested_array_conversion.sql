-- Fix upload_learning_path RPC to properly convert JSONB nested arrays to PostgreSQL integer[][]
-- Use array_agg and jsonb_array_elements for proper conversion
...
DECLARE
  ...
  new_deck_id TEXT;
  deck_index INT;
  transcript_line_ids_array integer[][];
BEGIN
  ...
  FOR deck_index IN 1..array_length(vocab_decks, 1) LOOP
    ...
    -- Convert JSONB nested array to PostgreSQL integer[][]
    -- Each element in the outer array is an array of line IDs for one word
    SELECT ARRAY(
      SELECT ARRAY(
        SELECT (inner_elem)::integer
        FROM jsonb_array_elements(outer_elem) AS inner_elem
      )
      FROM jsonb_array_elements(deck_item->'transcriptLineIds') AS outer_elem
    ) INTO transcript_line_ids_array;
  ...
  FOR module_index IN 1..array_length(module_sources, 1) LOOP
    ...
    -- Convert JSONB nested array to PostgreSQL integer[][] for grammar modules
    SELECT ARRAY(
      SELECT ARRAY(
        SELECT (inner_elem)::integer
        FROM jsonb_array_elements(outer_elem) AS inner_elem
      )
      FROM jsonb_array_elements(module_item->'transcriptLineIds') AS outer_elem
    ) INTO transcript_line_ids_array;
