import type { DynamicModuleCollection } from "./types"

export const dynamic_modules: DynamicModuleCollection = {
  "practice-hiragana": {
    id: "practice-hiragana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch0",
    name: "Hiragana Practice (Basic Set 1)",
    session_type: "vocab-practice",
    part_of_speech_category: "hiragana",
    // prettier-ignore
    ordered_vocab_keys: [
      "あ", "い", "う", "え", "お",  "か", "き", "く", "け", "こ",
      "さ", "し", "す", "せ", "そ",
    ],
    instructions: "Practice writing and recognizing these hiragana characters.",
  },
  "hiragana-quiz": {
    id: "hiragana-quiz",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch0",
    name: "Hiragana Quiz (Basic Set 1)",
    session_type: "vocab-test",
    part_of_speech_category: "hiragana",
    // prettier-ignore
    ordered_vocab_keys: [
      "あ", "い", "う", "え", "お",  "か", "き", "く", "け", "こ",
      "さ", "し", "す", "せ", "そ",
    ],
    instructions: "Test your knowledge of these hiragana characters.",
  },
  "practice-dakuten-handakuten": {
    id: "practice-dakuten-handakuten",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch0",
    name: "Hiragana Practice (Dakuten & Handakuten)",
    session_type: "vocab-practice",
    part_of_speech_category: "hiragana-voiced",
    // prettier-ignore
    ordered_vocab_keys: [
      "が", "ぎ", "ぐ", "げ", "ご",  "ざ", "じ", "ず", "ぜ", "ぞ",
      "だ", "ぢ", "づ", "で", "ど",  "ば", "び", "ぶ", "べ", "ぼ",
      "ぱ", "ぴ", "ぷ", "ぺ", "ぽ",
    ],
    instructions:
      "Practice voiced (dakuten) and semi-voiced (handakuten) hiragana.",
  },
  "contracted-sounds-quiz": {
    id: "contracted-sounds-quiz",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch0",
    name: "Hiragana Quiz (Contracted Sounds)",
    session_type: "vocab-test",
    part_of_speech_category: "hiragana-contracted",
    // prettier-ignore
    ordered_vocab_keys: [
      "きゃ", "きゅ", "きょ",  "しゃ", "しゅ", "しょ",  "ちゃ", "ちゅ", "ちょ",
      "にゃ", "にゅ", "にょ",  "ひゃ", "ひゅ", "ひょ",  "みゃ", "みゅ", "みょ",
      "りゃ", "りゅ", "りょ",
    ],
    instructions: "Test your knowledge of contracted hiragana sounds.",
  },
  "practice-contracted-sounds": {
    id: "practice-contracted-sounds",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch0",
    name: "Hiragana Practice (Contracted Sounds)",
    session_type: "vocab-practice",
    part_of_speech_category: "hiragana-contracted",
    // prettier-ignore
    ordered_vocab_keys: [
      "きゃ", "きゅ", "きょ",  "しゃ", "しゅ", "しょ",  "ちゃ", "ちゅ", "ちょ",
      "にゃ", "にゅ", "にょ",  "ひゃ", "ひゅ", "ひょ",  "みゃ", "みゅ", "みょ",
      "りゃ", "りゅ", "りょ",  "ぎゃ", "ぎゅ", "ぎょ",  "じゃ", "じゅ", "じょ",
      "びゃ", "びゅ", "びょ",  "ぴゃ", "ぴゅ", "ぴょ",
    ],
    instructions:
      "Practice recognizing and writing contracted hiragana sounds.",
  },
  "all-hiragana-quiz": {
    id: "all-hiragana-quiz",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch0",
    name: "Comprehensive Hiragana Quiz",
    session_type: "vocab-test",
    part_of_speech_category: "hiragana-all",
    // prettier-ignore
    ordered_vocab_keys: [
      "あ", "こ", "せ", "た", "ぬ",  "へ", "も", "や", "る", "を", "ん",
      "が", "じ", "づ", "べ", "ぽ",  "きゃ", "しゅ", "ちょ", "にゃ", "ひゅ",
      "みょ", "りゃ",
    ],
    instructions: "A comprehensive quiz covering all hiragana learned so far.",
  },
  "genki_1_ch0-vocab-list": {
    id: "genki_1_ch0-vocab-list",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch0",
    name: "Chapter 0 Vocabulary List",
    session_type: "vocab-list",
    // prettier-ignore
    ordered_vocab_keys: [
      // Get from textbook chapter vocab keys
    ],
    instructions: "Review the vocabulary for this introductory chapter.",
  },
  "genki_1_ch0_practice-greetings-common-expressions": {
    id: "genki_1_ch0_practice-greetings-common-expressions",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch0",
    name: "Practice: Greetings & Common Expressions",
    session_type: "vocab-practice",
    // prettier-ignore
    ordered_vocab_keys: [
      "おはようございます", "こんにちは", "こんばんは", "さようなら", "おやすみなさい",
      "はじめまして", "どうぞよろしくおねがいします", "こちらは〜さんです",
      "〜からきました", "はい", "いいえ", "そうです", "ちがいます", "あのう", "どうぞ",
    ],
    instructions: "Practice using these essential greetings and expressions.",
  },
  "genki_1_ch0_practice-numbers-0-10": {
    id: "genki_1_ch0_practice-numbers-0-10",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch0",
    name: "Practice: Numbers 0-10",
    session_type: "vocab-practice",
    part_of_speech_category: "number",
    // prettier-ignore
    ordered_vocab_keys: [
      "れい", "ゼロ", "いち", "に", "さん", "よん", "し", "ご",
      "ろく", "なな", "しち", "はち", "きゅう", "く", "じゅう",
    ],
    instructions: "Practice recognizing and saying numbers from 0 to 10.",
  },
  "genki_1_ch0_practice-numbers-11-100": {
    id: "genki_1_ch0_practice-numbers-11-100",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch0",
    name: "Practice: Numbers 11-100",
    session_type: "vocab-practice",
    part_of_speech_category: "number",
    // prettier-ignore
    ordered_vocab_keys: [
      "じゅういち", "じゅうご", "じゅうきゅう", "にじゅう", "にじゅうさん", "さんじゅう",
      "よんじゅうご", "ごじゅういち", "ろくじゅうなな", "ななじゅうに", "はちじゅうきゅう",
      "きゅうじゅう", "ひゃく",
    ],
    instructions: "Practice numbers from 11 to 100.",
  },

  // Chapter 1 Dynamic Modules
  "genki_1_ch1_vocab-list": {
    id: "genki_1_ch1_vocab-list",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch1",
    name: "Chapter 1 Vocab List",
    session_type: "vocab-list",
    ordered_vocab_keys: [],
    instructions: "Review the vocabulary for Chapter 1.",
  },
  "genki_1_ch1_practice-kanji-numbers": {
    id: "genki_1_ch1_practice-kanji-numbers",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch1",
    name: "Practice Kanji Numbers",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing and writing Kanji numbers.",
  },
  "genki_1_ch1_people-descriptors-misc": {
    id: "genki_1_ch1_people-descriptors-misc",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch1",
    name: "People, Descriptors, Misc. Vocab",
    session_type: "vocab-sublist", // This is a vocab lesson now
    ordered_vocab_keys: [],
    instructions:
      "Learn vocabulary related to people, descriptors, and miscellaneous terms.",
  },
  "genki_1_ch1_practice-people-descriptors-misc": {
    id: "genki_1_ch1_practice-people-descriptors-misc",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch1",
    name: "Practice People, Descriptors, Misc.",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice vocabulary related to people, descriptors, and miscellaneous terms.",
  },
  "genki_1_ch1_family-school": {
    id: "genki_1_ch1_family-school",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch1",
    name: "Family & School Vocab",
    session_type: "vocab-sublist", // Vocab lesson
    ordered_vocab_keys: [],
    instructions: "Learn vocabulary related to family and school.",
  },
  "genki_1_ch1_practice-family-school": {
    id: "genki_1_ch1_practice-family-school",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch1",
    name: "Practice Family & School",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice vocabulary related to family and school.",
  },
  "sentence-practice-x-wa-y-desu": {
    id: "sentence-practice-x-wa-y-desu",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch1",
    name: "X は Y です Practice Sentences",
    session_type: "sentence-practice",
    ordered_vocab_keys: ["genki_1/sentence-practice-x-wa-y-desu"],
    instructions:
      "Practice creating and understanding sentences using the pattern X は Y です.",
  },
  "genki_1_ch1_occupations-majors": {
    id: "genki_1_ch1_occupations-majors",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch1",
    name: "Occupations & Majors Vocab",
    session_type: "vocab-sublist", // Vocab lesson
    ordered_vocab_keys: [],
    instructions: "Learn vocabulary related to occupations and majors.",
  },
  "genki_1_ch1_practice-occupations-majors": {
    id: "genki_1_ch1_practice-occupations-majors",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch1",
    name: "Practice Occupations & Majors",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice vocabulary related to occupations and majors.",
  },
  "genki_1_ch1_countries-time": {
    id: "genki_1_ch1_countries-time",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch1",
    name: "Countries & Time Vocab",
    session_type: "vocab-sublist", // Vocab lesson
    ordered_vocab_keys: [],
    instructions: "Learn vocabulary related to countries and time.",
  },
  "genki_1_ch1_practice-countries-time": {
    id: "genki_1_ch1_practice-countries-time",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch1",
    name: "Practice Countries & Time",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice vocabulary related to countries and time.",
  },
  "sentence-practice-questions-with-ka": {
    id: "sentence-practice-questions-with-ka",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch1",
    name: "Practice Questions with か",
    session_type: "sentence-practice",
    ordered_vocab_keys: ["genki_1/sentence-practice-questions-with-ka"],
    instructions: "Practice forming and answering questions using か.",
  },
  "sentence-practice-the-no-particle": {
    id: "sentence-practice-the-no-particle",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch1",
    name: "Practice Modifying Nouns: の",
    session_type: "sentence-practice",
    ordered_vocab_keys: ["genki_1/sentence-practice-the-no-particle"],
    instructions: "Practice using the particle の to modify nouns.",
  },
  "genki_1_ch1_practice-telling-time": {
    id: "genki_1_ch1_practice-telling-time",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch1",
    name: "Practice Time",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice telling time in Japanese.",
  },
  "genki_1_ch1_practice-minutes": {
    id: "genki_1_ch1_practice-minutes",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch1",
    name: "Practice Minutes",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice counting minutes in Japanese.",
  },

  // Chapter 2 Dynamic Modules
  "genki_1_ch2_vocab-list": {
    id: "genki_1_ch2_vocab-list",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch2",
    name: "Chapter 2 Vocab List",
    session_type: "vocab-list",
    ordered_vocab_keys: [],
    instructions: "Review the vocabulary for Chapter 2.",
  },
  "practice-katakana": {
    id: "practice-katakana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch2",
    name: "Practice Katakana",
    session_type: "vocab-practice",
    part_of_speech_category: "katakana",
    ordered_vocab_keys: [],
    instructions: "Practice writing and recognizing Katakana characters.",
  },
  "katakana-quiz": {
    id: "katakana-quiz",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch2",
    name: "Katakana Quiz",
    session_type: "vocab-test",
    part_of_speech_category: "katakana",
    ordered_vocab_keys: [],
    instructions: "Test your knowledge of Katakana characters.",
  },
  genki_1_ch2_things: {
    id: "genki_1_ch2_things",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch2",
    name: "Things Vocab",
    session_type: "vocab-sublist", // Vocab lesson
    ordered_vocab_keys: [],
    instructions: "Learn vocabulary for common objects.",
  },
  "genki_1_ch2_practice-things": {
    id: "genki_1_ch2_practice-things",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch2",
    name: "Practice Things",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice vocabulary for common objects.",
  },
  "sentence-practice-words-that-point": {
    id: "sentence-practice-words-that-point",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch2",
    name: "Practice Words That Point Sentences",
    session_type: "sentence-practice",
    ordered_vocab_keys: ["genki_1/sentence-practice-words-that-point"],
    instructions:
      "Practice creating and understanding sentences using demonstrative pronouns.",
  },
  "sentence-practice-dare": {
    id: "sentence-practice-dare",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch2",
    name: "Practice だれ",
    session_type: "sentence-practice",
    ordered_vocab_keys: ["genki_1/sentence-practice-dare"],
    instructions: "Practice asking questions using だれ.",
  },
  "sentence-practice-mo-particle": {
    id: "sentence-practice-mo-particle",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch2",
    name: "Practice Saying also with も",
    session_type: "sentence-practice",
    ordered_vocab_keys: ["genki_1/sentence-practice-mo-particle"],
    instructions: "Practice using the particle も to mean 'also' or 'too'.",
  },
  "sentence-practice-janai": {
    id: "sentence-practice-janai",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch2",
    name: "Practice じゃないです - Is not...",
    session_type: "sentence-practice",
    ordered_vocab_keys: ["genki_1/sentence-practice-janai"],
    instructions: "Practice forming negative statements using じゃないです.",
  },
  "sentence-practice-ne-yo": {
    id: "sentence-practice-ne-yo",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch2",
    name: "Practice ね, よ",
    session_type: "sentence-practice",
    ordered_vocab_keys: ["genki_1/sentence-practice-ne-yo"],
    instructions:
      "Practice using the particles ね and よ to soften or emphasize statements.",
  },
  "genki_1_ch2_places-money-food": {
    id: "genki_1_ch2_places-money-food",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch2",
    name: "Places, Money, Food Vocab",
    session_type: "vocab-sublist", // Vocab lesson
    ordered_vocab_keys: [],
    instructions: "Learn vocabulary related to places, money, and food.",
  },
  "genki_1_ch2_practice-places-money-food": {
    id: "genki_1_ch2_practice-places-money-food",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch2",
    name: "Practice Places, Money, Food",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice vocabulary related to places, money, and food.",
  },
  "genki_1_ch2_all-vocab-test": {
    id: "genki_1_ch2_all-vocab-test",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch2",
    name: "All Vocab - Test",
    session_type: "vocab-test",
    ordered_vocab_keys: [],
    instructions: "Test your knowledge of all vocabulary from Chapter 2.",
  },

  // Chapter 3 Dynamic Modules
  "genki_1_ch3_vocab-list": {
    id: "genki_1_ch3_vocab-list",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch3",
    name: "Chapter 3 Vocab List",
    session_type: "vocab-list",
    ordered_vocab_keys: [],
    instructions: "Review the vocabulary for Chapter 3.",
  },
  "genki_1_ch3_practice-chapter-1-kanji-part-1": {
    id: "genki_1_ch3_practice-chapter-1-kanji-part-1",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch3",
    name: "Practice Chapter 1 Kanji Part 1",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice Kanji from Chapter 1, Part 1.",
  },
  "genki_1_ch3_practice-chapter-1-kanji-part-2": {
    id: "genki_1_ch3_practice-chapter-1-kanji-part-2",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch3",
    name: "Practice Chapter 1 Kanji Part 2",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice Kanji from Chapter 1, Part 2.",
  },
  "genki_1_ch3_practice-chapter-1-kanji-part-3": {
    id: "genki_1_ch3_practice-chapter-1-kanji-part-3",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch3",
    name: "Practice Chapter 1 Kanji Part 3",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice Kanji from Chapter 1, Part 3.",
  },
  "genki_1_ch3_practice-chapter-2-kanji": {
    id: "genki_1_ch3_practice-chapter-2-kanji",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch3",
    name: "Practice Chapter 2 Kanji",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice Kanji from Chapter 2.",
  },
  genki_1_ch3_nouns: {
    id: "genki_1_ch3_nouns",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch3",
    name: "Chapter 3 Nouns Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn nouns from Chapter 3.",
  },
  "genki_1_ch3_practice-nouns-readings": {
    id: "genki_1_ch3_practice-nouns-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch3",
    name: "Practice Nouns - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of nouns from Chapter 3.",
  },
  "genki_1_ch3_practice-nouns-kana": {
    id: "genki_1_ch3_practice-nouns-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch3",
    name: "Practice Nouns - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing nouns from Chapter 3 in Kana.",
  },
  "genki_1_ch3_days-and-time": {
    id: "genki_1_ch3_days-and-time",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch3",
    name: "Days of The Week & Time Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn vocabulary for days of the week and time.",
  },
  "genki_1_ch3_practice-days-and-time-readings": {
    id: "genki_1_ch3_practice-days-and-time-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch3",
    name: "Practice Days & Time - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of days and time vocabulary.",
  },
  "genki_1_ch3_practice-days-and-time-kana": {
    id: "genki_1_ch3_practice-days-and-time-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch3",
    name: "Practice Days & Time - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing days and time vocabulary in Kana.",
  },
  "genki_1_ch3_verbs-and-adj": {
    id: "genki_1_ch3_verbs-and-adj",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch3",
    name: "Chapter 3 Verbs & Adj. Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn verbs and adjectives from Chapter 3.",
  },
  "genki_1_ch3_practice-verbs-and-adj-readings": {
    id: "genki_1_ch3_practice-verbs-and-adj-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch3",
    name: "Practice Verbs & Adj. - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice the readings of verbs and adjectives from Chapter 3.",
  },
  "genki_1_ch3_practice-verbs-and-adj-kana": {
    id: "genki_1_ch3_practice-verbs-and-adj-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch3",
    name: "Practice Verbs & Adj. - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice recognizing verbs and adjectives from Chapter 3 in Kana.",
  },
  "sentence-practice-o-de-ni-e-particles": {
    id: "sentence-practice-o-de-ni-e-particles",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch3",
    name: "Practice Particles - を, で, に, へ",
    session_type: "sentence-practice",
    ordered_vocab_keys: ["genki_1/sentence-practice-o-de-ni-e-particles"],
    instructions:
      "Practice using the particles を, で, に, and へ in sentences.",
  },
  "sentence-practice-polite-invitations": {
    id: "sentence-practice-polite-invitations",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch3",
    name: "Practice Polite Invitations - ませんか",
    session_type: "sentence-practice",
    ordered_vocab_keys: ["genki_1/sentence-practice-polite-invitations"],
    instructions: "Practice forming and responding to polite invitations.",
  },
  "genki_1_ch3_adverbs-expressions": {
    id: "genki_1_ch3_adverbs-expressions",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch3",
    name: "Adverbs & Expressions Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn adverbs and expressions from Chapter 3.",
  },
  "genki_1_ch3_practice-adverbs-expressions-readings": {
    id: "genki_1_ch3_practice-adverbs-expressions-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch3",
    name: "Practice Adv. & Expres. - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice the readings of adverbs and expressions from Chapter 3.",
  },
  "genki_1_ch3_practice-adverbs-expressions-kana": {
    id: "genki_1_ch3_practice-adverbs-expressions-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch3",
    name: "Practice Adv. & Expres. - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice recognizing adverbs and expressions from Chapter 3 in Kana.",
  },
  "genki_1_ch3_all-vocab-test": {
    id: "genki_1_ch3_all-vocab-test",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch3",
    name: "All Vocab - Test",
    session_type: "vocab-test",
    ordered_vocab_keys: [],
    instructions: "Test your knowledge of all vocabulary from Chapter 3.",
  },
  "sentence-practice-time-expressions": {
    id: "sentence-practice-time-expressions",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch3",
    name: "Practice Time Expressions",
    session_type: "sentence-practice",
    ordered_vocab_keys: ["genki_1/sentence-practice-time-expressions"],
    instructions: "Practice using time expressions in sentences.",
  },
  "sentence-practice-frequency-adverbs": {
    id: "sentence-practice-frequency-adverbs",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch3",
    name: "Practice Frequency Adverbs",
    session_type: "sentence-practice",
    ordered_vocab_keys: ["genki_1/sentence-practice-frequency-adverbs"],
    instructions: "Practice using frequency adverbs in sentences.",
  },
  "genki_1_ch4_vocab-list": {
    id: "genki_1_ch4_vocab-list",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch4",
    name: "Chapter 4 Vocab List",
    session_type: "vocab-list",
    ordered_vocab_keys: [],
    instructions: "Review the vocabulary for Chapter 4.",
  },
  "genki_1_ch4_nouns-1": {
    id: "genki_1_ch4_nouns-1",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch4",
    name: "Nouns 1 Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn the first set of nouns from Chapter 4.",
  },
  "genki_1_ch4_practice-nouns-1-readings": {
    id: "genki_1_ch4_practice-nouns-1-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch4",
    name: "Practice Nouns 1 - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of the first set of nouns.",
  },
  "genki_1_ch4_practice-nouns-1-kana": {
    id: "genki_1_ch4_practice-nouns-1-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch4",
    name: "Practice Nouns 1 - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing the first set of nouns in Kana.",
  },
  "genki_1_ch4_nouns-2": {
    id: "genki_1_ch4_nouns-2",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch4",
    name: "Nouns 2 Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn the second set of nouns from Chapter 4.",
  },
  "genki_1_ch4_practice-nouns-2-readings": {
    id: "genki_1_ch4_practice-nouns-2-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch4",
    name: "Practice Nouns 2 - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of the second set of nouns.",
  },
  "genki_1_ch4_practice-nouns-2-kana": {
    id: "genki_1_ch4_practice-nouns-2-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch4",
    name: "Practice Nouns 2 - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing the second set of nouns in Kana.",
  },
  "genki_1_ch4_location-words": {
    id: "genki_1_ch4_location-words",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch4",
    name: "Location Words Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn vocabulary for indicating location.",
  },
  "genki_1_ch4_practice-location-words-readings": {
    id: "genki_1_ch4_practice-location-words-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch4",
    name: "Practice Location Words - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of location words.",
  },
  "genki_1_ch4_practice-location-words-kana": {
    id: "genki_1_ch4_practice-location-words-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch4",
    name: "Practice Location Words - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing location words in Kana.",
  },
  "sentence-practice-iru-aru": {
    id: "sentence-practice-iru-aru",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch4",
    name: "Practice いる・ある",
    session_type: "sentence-practice",
    ordered_vocab_keys: ["genki_1/sentence-practice-iru-aru"],
    instructions: "Practice using いる and ある to express existence.",
  },
  "sentence-practice-where-things-are": {
    id: "sentence-practice-where-things-are",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch4",
    name: "Practice Where Things Are",
    session_type: "sentence-practice",
    ordered_vocab_keys: ["genki_1/sentence-practice-where-things-are"],
    instructions:
      "Practice forming sentences indicating the location of things.",
  },
  "genki_1_ch4_verbs-adv-misc": {
    id: "genki_1_ch4_verbs-adv-misc",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch4",
    name: "Verbs, Adv, & Misc. Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions:
      "Learn verbs, adverbs, and miscellaneous vocabulary from Chapter 4.",
  },
  "genki_1_ch4_practice-verbs-adv-misc-readings": {
    id: "genki_1_ch4_practice-verbs-adv-misc-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch4",
    name: "Practice V, Adv, & Misc. - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice the readings of verbs, adverbs, and miscellaneous vocabulary.",
  },
  "genki_1_ch4_practice-verbs-adv-misc-kana": {
    id: "genki_1_ch4_practice-verbs-adv-misc-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch4",
    name: "Practice V, Adv, & Misc. - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice recognizing verbs, adverbs, and miscellaneous vocabulary in Kana.",
  },
  "genki_1_ch4_all-vocab-test": {
    id: "genki_1_ch4_all-vocab-test",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch4",
    name: "All Vocab - Test",
    session_type: "vocab-test",
    ordered_vocab_keys: [],
    instructions: "Test your knowledge of all vocabulary from Chapter 4.",
  },
  "sentence-practice-deshita-past-tense-verbs": {
    id: "sentence-practice-deshita-past-tense-verbs",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch4",
    name: "Practice でした + Past-Tense Verbs",
    session_type: "sentence-practice",
    ordered_vocab_keys: ["genki_1/sentence-practice-deshita-past-tense-verbs"],
    instructions: "Practice using でした and past-tense verbs in sentences.",
  },
  "sentence-practice-to-particle": {
    id: "sentence-practice-to-particle",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch4",
    name: "Practice と",
    session_type: "sentence-practice",
    ordered_vocab_keys: ["genki_1/sentence-practice-to-particle"],
    instructions: "Practice using the particle と to mean 'and' or 'with'.",
  },
  "sentence-practice-duration": {
    id: "sentence-practice-duration",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch4",
    name: "Practice Time Duration",
    session_type: "sentence-practice",
    ordered_vocab_keys: ["genki_1/sentence-practice-duration"],
    instructions: "Practice expressing durations of time.",
  },

  // Chapter 5 Dynamic Modules
  "genki_1_ch5_vocab-list": {
    id: "genki_1_ch5_vocab-list",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch5",
    name: "Chapter 5 Vocab List",
    session_type: "vocab-list",
    ordered_vocab_keys: [],
    instructions: "Review the vocabulary for Chapter 5.",
  },
  genki_1_ch5_nouns: {
    id: "genki_1_ch5_nouns",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch5",
    name: "Nouns 1 Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn the nouns from Chapter 5.",
  },
  "genki_1_ch5_practice-nouns-readings": {
    id: "genki_1_ch5_practice-nouns-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch5",
    name: "Practice Nouns 1 - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of nouns from Chapter 5.",
  },
  "genki_1_ch5_practice-nouns-kana": {
    id: "genki_1_ch5_practice-nouns-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch5",
    name: "Practice Nouns 1 - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing nouns from Chapter 5 in Kana.",
  },
  "genki_1_ch5_i-adjectives": {
    id: "genki_1_ch5_i-adjectives",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch5",
    name: "い Adjectives Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn い-adjectives from Chapter 5.",
  },
  "genki_1_ch5_practice-i-adjectives-readings": {
    id: "genki_1_ch5_practice-i-adjectives-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch5",
    name: "Practice い Adjectives - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of い-adjectives.",
  },
  "genki_1_ch5_practice-i-adjectives-kana": {
    id: "genki_1_ch5_practice-i-adjectives-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch5",
    name: "Practice い Adjectives - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing い-adjectives in Kana.",
  },
  "genki_1_ch5_na-adjectives": {
    id: "genki_1_ch5_na-adjectives",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch5",
    name: "な Adjectives Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn な-adjectives from Chapter 5.",
  },
  "genki_1_ch5_practice-na-adjectives-readings": {
    id: "genki_1_ch5_practice-na-adjectives-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch5",
    name: "Practice な Adjectives - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of な-adjectives.",
  },
  "genki_1_ch5_practice-na-adjectives-kana": {
    id: "genki_1_ch5_practice-na-adjectives-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch5",
    name: "Practice な Adjectives - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing な-adjectives in Kana.",
  },
  "sentence-practice-adjectives": {
    id: "sentence-practice-adjectives",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch5",
    name: "Practice Adjectives in Sentences",
    session_type: "sentence-practice",
    ordered_vocab_keys: ["genki_1/sentence-practice-adjectives"],
    instructions: "Practice using adjectives in sentences.",
  },
  "sentence-practice-suki-kirai": {
    id: "sentence-practice-suki-kirai",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch5",
    name: "Practice 好き・嫌い Sentences",
    session_type: "sentence-practice",
    ordered_vocab_keys: ["genki_1/sentence-practice-suki-kirai"],
    instructions: "Practice forming sentences expressing likes and dislikes.",
  },
  "genki_1_ch5_verbs-adv-misc": {
    id: "genki_1_ch5_verbs-adv-misc",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch5",
    name: "Verbs, Adv, & Misc. Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions:
      "Learn verbs, adverbs, and miscellaneous vocabulary from Chapter 5.",
  },
  "genki_1_ch5_practice-verbs-adv-misc-readings": {
    id: "genki_1_ch5_practice-verbs-adv-misc-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch5",
    name: "Practice Verbs, Adv, & Misc. - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice the readings of verbs, adverbs, and miscellaneous vocabulary.",
  },
  "genki_1_ch5_practice-verbs-adv-misc-kana": {
    id: "genki_1_ch5_practice-verbs-adv-misc-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch5",
    name: "Practice Verbs, Adv, & Misc. - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice recognizing verbs, adverbs, and miscellaneous vocabulary in Kana.",
  },
  "genki_1_ch5_all-vocab-test": {
    id: "genki_1_ch5_all-vocab-test",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch5",
    name: "All Vocab - Test",
    session_type: "vocab-test",
    ordered_vocab_keys: [],
    instructions: "Test your knowledge of all vocabulary from Chapter 5.",
  },

  // Chapter 6 Dynamic Modules
  "genki_1_ch6_vocab-list": {
    id: "genki_1_ch6_vocab-list",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch6",
    name: "Chapter 6 Vocab List",
    session_type: "vocab-list",
    ordered_vocab_keys: [],
    instructions: "Review the vocabulary for Chapter 6.",
  },
  genki_1_ch6_nouns: {
    id: "genki_1_ch6_nouns",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch6",
    name: "Nouns Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn nouns from Chapter 6.",
  },
  "genki_1_ch6_practice-nouns-readings": {
    id: "genki_1_ch6_practice-nouns-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch6",
    name: "Practice Nouns - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of nouns from Chapter 6.",
  },
  "genki_1_ch6_practice-nouns-kana": {
    id: "genki_1_ch6_practice-nouns-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch6",
    name: "Practice Nouns - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing nouns from Chapter 6 in Kana.",
  },
  "genki_1_ch6_u-verbs-and-adj": {
    id: "genki_1_ch6_u-verbs-and-adj",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch6",
    name: "U-Verbs & Adjectives Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn U-verbs and adjectives from Chapter 6.",
  },
  "genki_1_ch6_practice-u-verbs-and-adj-readings": {
    id: "genki_1_ch6_practice-u-verbs-and-adj-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch6",
    name: "Practice U-V & Adj - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of U-verbs and adjectives.",
  },
  "genki_1_ch6_practice-u-verbs-and-adj-kana": {
    id: "genki_1_ch6_practice-u-verbs-and-adj-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch6",
    name: "Practice U-V & Adj - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing U-verbs and adjectives in Kana.",
  },
  "genki_1_ch6_ru-v-irr-v-adv-misc": {
    id: "genki_1_ch6_ru-v-irr-v-adv-misc",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch6",
    name: "Ru-Verbs, Irr-Verbs, & Misc. Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions:
      "Learn Ru-verbs, irregular verbs, and miscellaneous vocabulary from Chapter 6.",
  },
  "genki_1_ch6_practice-ru-v-irr-v-adv-misc-readings": {
    id: "genki_1_ch6_practice-ru-v-irr-v-adv-misc-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch6",
    name: "Practice Ru-V, Irr-V, & Misc. - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice the readings of Ru-verbs, irregular verbs, and miscellaneous vocabulary.",
  },
  "genki_1_ch6_practice-ru-v-irr-v-adv-misc-kana": {
    id: "genki_1_ch6_practice-ru-v-irr-v-adv-misc-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch6",
    name: "Practice Ru-V, Irr-V, & Misc. - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice recognizing Ru-verbs, irregular verbs, and miscellaneous vocabulary in Kana.",
  },
  "genki_1_ch6_all-vocab-test": {
    id: "genki_1_ch6_all-vocab-test",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch6",
    name: "All Vocab - Test",
    session_type: "vocab-test",
    ordered_vocab_keys: [],
    instructions: "Test your knowledge of all vocabulary from Chapter 6.",
  },

  // Chapter 7 Dynamic Modules
  "genki_1_ch7_vocab-list": {
    id: "genki_1_ch7_vocab-list",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch7",
    name: "Chapter 7 Vocab List",
    session_type: "vocab-list",
    ordered_vocab_keys: [],
    instructions: "Review the vocabulary for Chapter 7.",
  },
  "genki_1_ch7_nouns-1": {
    id: "genki_1_ch7_nouns-1",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch7",
    name: "Nouns 1 Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn the first set of nouns from Chapter 7.",
  },
  "genki_1_ch7_practice-nouns-1-readings": {
    id: "genki_1_ch7_practice-nouns-1-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch7",
    name: "Practice Nouns 1 - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of the first set of nouns.",
  },
  "genki_1_ch7_practice-nouns-1-kana": {
    id: "genki_1_ch7_practice-nouns-1-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch7",
    name: "Practice Nouns 1 - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing the first set of nouns in Kana.",
  },
  "genki_1_ch7_body-parts": {
    id: "genki_1_ch7_body-parts",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch7",
    name: "Body Parts Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn vocabulary for body parts.",
  },
  "genki_1_ch7_practice-body-parts-readings": {
    id: "genki_1_ch7_practice-body-parts-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch7",
    name: "Practice Body Parts - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of body parts vocabulary.",
  },
  "genki_1_ch7_practice-body-parts-kana": {
    id: "genki_1_ch7_practice-body-parts-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch7",
    name: "Practice Body Parts - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing body parts vocabulary in Kana.",
  },
  genki_1_ch7_verbs: {
    id: "genki_1_ch7_verbs",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch7",
    name: "Verbs Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn verbs from Chapter 7.",
  },
  "genki_1_ch7_practice-verbs-readings": {
    id: "genki_1_ch7_practice-verbs-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch7",
    name: "Practice Verbs - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of verbs from Chapter 7.",
  },
  "genki_1_ch7_practice-verbs-kana": {
    id: "genki_1_ch7_practice-verbs-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch7",
    name: "Practice Verbs - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing verbs from Chapter 7 in Kana.",
  },
  "genki_1_ch7_adj-adv-misc": {
    id: "genki_1_ch7_adj-adv-misc",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch7",
    name: "Adj, Adv, & Misc. Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions:
      "Learn adjectives, adverbs, and miscellaneous vocabulary from Chapter 7.",
  },
  "genki_1_ch7_practice-adj-adv-misc-readings": {
    id: "genki_1_ch7_practice-adj-adv-misc-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch7",
    name: "Practice Adj, Adv, & Misc. - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice the readings of adjectives, adverbs, and miscellaneous vocabulary.",
  },
  "genki_1_ch7_practice-adj-adv-misc-kana": {
    id: "genki_1_ch7_practice-adj-adv-misc-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch7",
    name: "Practice Adj, Adv, & Misc. - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice recognizing adjectives, adverbs, and miscellaneous vocabulary in Kana.",
  },
  "genki_1_ch7_all-vocab-test": {
    id: "genki_1_ch7_all-vocab-test",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch7",
    name: "All Vocab - Test",
    session_type: "vocab-test",
    ordered_vocab_keys: [],
    instructions: "Test your knowledge of all vocabulary from Chapter 7.",
  },

  // Chapter 8 Dynamic Modules
  "genki_1_ch8_vocab-list": {
    id: "genki_1_ch8_vocab-list",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch8",
    name: "Chapter 8 Vocab List",
    session_type: "vocab-list",
    ordered_vocab_keys: [],
    instructions: "Review the vocabulary for Chapter 8.",
  },
  "genki_1_ch8_nouns-1": {
    id: "genki_1_ch8_nouns-1",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch8",
    name: "Nouns 1 Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn the first set of nouns from Chapter 8.",
  },
  "genki_1_ch8_practice-nouns-1-readings": {
    id: "genki_1_ch8_practice-nouns-1-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch8",
    name: "Practice Nouns 1 - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of the first set of nouns.",
  },
  "genki_1_ch8_practice-nouns-1-kana": {
    id: "genki_1_ch8_practice-nouns-1-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch8",
    name: "Practice Nouns 1 - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing the first set of nouns in Kana.",
  },
  "genki_1_ch8_nouns-2": {
    id: "genki_1_ch8_nouns-2",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch8",
    name: "Nouns 2 Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn the second set of nouns from Chapter 8.",
  },
  "genki_1_ch8_practice-nouns-2-readings": {
    id: "genki_1_ch8_practice-nouns-2-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch8",
    name: "Practice Nouns 2 - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of the second set of nouns.",
  },
  "genki_1_ch8_practice-nouns-2-kana": {
    id: "genki_1_ch8_practice-nouns-2-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch8",
    name: "Practice Nouns 2 - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing the second set of nouns in Kana.",
  },
  genki_1_ch8_verbs: {
    id: "genki_1_ch8_verbs",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch8",
    name: "Verbs Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn verbs from Chapter 8.",
  },
  "genki_1_ch8_practice-verbs-readings": {
    id: "genki_1_ch8_practice-verbs-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch8",
    name: "Practice Verbs - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of verbs from Chapter 8.",
  },
  "genki_1_ch8_practice-verbs-kana": {
    id: "genki_1_ch8_practice-verbs-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch8",
    name: "Practice Verbs - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing verbs from Chapter 8 in Kana.",
  },
  "genki_1_ch8_adj-adv-misc": {
    id: "genki_1_ch8_adj-adv-misc",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch8",
    name: "Adj, Adv, & Misc. Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions:
      "Learn adjectives, adverbs, and miscellaneous vocabulary from Chapter 8.",
  },
  "genki_1_ch8_practice-adj-adv-misc-readings": {
    id: "genki_1_ch8_practice-adj-adv-misc-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch8",
    name: "Practice Adj, Adv, & Misc. - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice the readings of adjectives, adverbs, and miscellaneous vocabulary.",
  },
  "genki_1_ch8_practice-adj-adv-misc-kana": {
    id: "genki_1_ch8_practice-adj-adv-misc-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch8",
    name: "Practice Adj, Adv, & Misc. - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice recognizing adjectives, adverbs, and miscellaneous vocabulary in Kana.",
  },
  "genki_1_ch8_all-vocab-test": {
    id: "genki_1_ch8_all-vocab-test",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch8",
    name: "All Vocab - Test",
    session_type: "vocab-test",
    ordered_vocab_keys: [],
    instructions: "Test your knowledge of all vocabulary from Chapter 8.",
  },

  // Chapter 9 Dynamic Modules
  "genki_1_ch9_vocab-list": {
    id: "genki_1_ch9_vocab-list",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch9",
    name: "Chapter 9 Vocab List",
    session_type: "vocab-list",
    ordered_vocab_keys: [],
    instructions: "Review the vocabulary for Chapter 9.",
  },
  genki_1_ch9_nouns: {
    id: "genki_1_ch9_nouns",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch9",
    name: "Nouns Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn nouns from Chapter 9.",
  },
  "genki_1_ch9_practice-nouns-readings": {
    id: "genki_1_ch9_practice-nouns-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch9",
    name: "Practice Nouns - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of nouns from Chapter 9.",
  },
  "genki_1_ch9_practice-nouns-kana": {
    id: "genki_1_ch9_practice-nouns-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch9",
    name: "Practice Nouns - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing nouns from Chapter 9 in Kana.",
  },
  "genki_1_ch9_small-item-counters": {
    id: "genki_1_ch9_small-item-counters",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch9",
    name: "Small Item Counters Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn counters for small items.",
  },
  "genki_1_ch9_practice-small-item-counters-readings": {
    id: "genki_1_ch9_practice-small-item-counters-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch9",
    name: "Practice Small Item Counters - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of small item counters.",
  },
  "genki_1_ch9_practice-small-item-counters-kana": {
    id: "genki_1_ch9_practice-small-item-counters-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch9",
    name: "Practice Small Item Counters - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing small item counters in Kana.",
  },
  genki_1_ch9_verbs: {
    id: "genki_1_ch9_verbs",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch9",
    name: "Verbs Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn verbs from Chapter 9.",
  },
  "genki_1_ch9_practice-verbs-readings": {
    id: "genki_1_ch9_practice-verbs-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch9",
    name: "Practice Verbs - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of verbs from Chapter 9.",
  },
  "genki_1_ch9_practice-verbs-kana": {
    id: "genki_1_ch9_practice-verbs-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch9",
    name: "Practice Verbs - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing verbs from Chapter 9 in Kana.",
  },
  "genki_1_ch9_adj-adv-misc": {
    id: "genki_1_ch9_adj-adv-misc",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch9",
    name: "Adj, Adv, & Misc. Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions:
      "Learn adjectives, adverbs, and miscellaneous vocabulary from Chapter 9.",
  },
  "genki_1_ch9_practice-adj-adv-misc-readings": {
    id: "genki_1_ch9_practice-adj-adv-misc-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch9",
    name: "Practice Adj, Adv, & Misc. - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice the readings of adjectives, adverbs, and miscellaneous vocabulary.",
  },
  "genki_1_ch9_practice-adj-adv-misc-kana": {
    id: "genki_1_ch9_practice-adj-adv-misc-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch9",
    name: "Practice Adj, Adv, & Misc. - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice recognizing adjectives, adverbs, and miscellaneous vocabulary in Kana.",
  },
  "genki_1_ch9_all-vocab-test": {
    id: "genki_1_ch9_all-vocab-test",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch9",
    name: "All Vocab - Test",
    session_type: "vocab-test",
    ordered_vocab_keys: [],
    instructions: "Test your knowledge of all vocabulary from Chapter 9.",
  },

  // Chapter 10 Dynamic Modules
  "genki_1_ch10_vocab-list": {
    id: "genki_1_ch10_vocab-list",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch10",
    name: "Chapter 10 Vocab List",
    session_type: "vocab-list",
    ordered_vocab_keys: [],
    instructions: "Review the vocabulary for Chapter 10.",
  },
  "genki_1_ch10_nouns-1": {
    id: "genki_1_ch10_nouns-1",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch10",
    name: "Nouns 1 Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn the first set of nouns from Chapter 10.",
  },
  "genki_1_ch10_practice-nouns-1-readings": {
    id: "genki_1_ch10_practice-nouns-1-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch10",
    name: "Practice Nouns 1 - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of the first set of nouns.",
  },
  "genki_1_ch10_practice-nouns-1-kana": {
    id: "genki_1_ch10_practice-nouns-1-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch10",
    name: "Practice Nouns 1 - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing the first set of nouns in Kana.",
  },
  "genki_1_ch10_nouns-2": {
    id: "genki_1_ch10_nouns-2",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch10",
    name: "Nouns 2 Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn the second set of nouns from Chapter 10.",
  },
  "genki_1_ch10_practice-nouns-2-readings": {
    id: "genki_1_ch10_practice-nouns-2-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch10",
    name: "Practice Nouns 2 - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of the second set of nouns.",
  },
  "genki_1_ch10_practice-nouns-2-kana": {
    id: "genki_1_ch10_practice-nouns-2-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch10",
    name: "Practice Nouns 2 - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing the second set of nouns in Kana.",
  },
  genki_1_ch10_verbs: {
    id: "genki_1_ch10_verbs",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch10",
    name: "Verbs Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn verbs from Chapter 10.",
  },
  "genki_1_ch10_practice-verbs-readings": {
    id: "genki_1_ch10_practice-verbs-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch10",
    name: "Practice Verbs - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of verbs from Chapter 10.",
  },
  "genki_1_ch10_practice-verbs-kana": {
    id: "genki_1_ch10_practice-verbs-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch10",
    name: "Practice Verbs - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing verbs from Chapter 10 in Kana.",
  },
  "genki_1_ch10_adj-adv-misc": {
    id: "genki_1_ch10_adj-adv-misc",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch10",
    name: "Adj, Adv, & Misc. Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions:
      "Learn adjectives, adverbs, and miscellaneous vocabulary from Chapter 10.",
  },
  "genki_1_ch10_practice-adj-adv-misc-readings": {
    id: "genki_1_ch10_practice-adj-adv-misc-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch10",
    name: "Practice Adj, Adv, & Misc. - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice the readings of adjectives, adverbs, and miscellaneous vocabulary.",
  },
  "genki_1_ch10_practice-adj-adv-misc-kana": {
    id: "genki_1_ch10_practice-adj-adv-misc-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch10",
    name: "Practice Adj, Adv, & Misc. - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice recognizing adjectives, adverbs, and miscellaneous vocabulary in Kana.",
  },
  "genki_1_ch10_all-vocab-test": {
    id: "genki_1_ch10_all-vocab-test",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch10",
    name: "All Vocab - Test",
    session_type: "vocab-test",
    ordered_vocab_keys: [],
    instructions: "Test your knowledge of all vocabulary from Chapter 10.",
  },

  // Chapter 11 Dynamic Modules
  "genki_1_ch11_vocab-list": {
    id: "genki_1_ch11_vocab-list",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch11",
    name: "Chapter 11 Vocab List",
    session_type: "vocab-list",
    ordered_vocab_keys: [],
    instructions: "Review the vocabulary for Chapter 11.",
  },
  "genki_1_ch11_nouns-1": {
    id: "genki_1_ch11_nouns-1",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch11",
    name: "Nouns 1 Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn the first set of nouns from Chapter 11.",
  },
  "genki_1_ch11_practice-nouns-1-readings": {
    id: "genki_1_ch11_practice-nouns-1-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch11",
    name: "Practice Nouns 1 - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of the first set of nouns.",
  },
  "genki_1_ch11_practice-nouns-1-kana": {
    id: "genki_1_ch11_practice-nouns-1-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch11",
    name: "Practice Nouns 1 - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing the first set of nouns in Kana.",
  },
  "genki_1_ch11_nouns-2": {
    id: "genki_1_ch11_nouns-2",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch11",
    name: "Nouns 2 Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn the second set of nouns from Chapter 11.",
  },
  "genki_1_ch11_practice-nouns-2-readings": {
    id: "genki_1_ch11_practice-nouns-2-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch11",
    name: "Practice Nouns 2 - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of the second set of nouns.",
  },
  "genki_1_ch11_practice-nouns-2-kana": {
    id: "genki_1_ch11_practice-nouns-2-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch11",
    name: "Practice Nouns 2 - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing the second set of nouns in Kana.",
  },
  "genki_1_ch11_ru-v-u-v": {
    id: "genki_1_ch11_ru-v-u-v",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch11",
    name: "Ru and U-Verbs Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn Ru-verbs and U-verbs from Chapter 11.",
  },
  "genki_1_ch11_practice-ru-v-u-v-readings": {
    id: "genki_1_ch11_practice-ru-v-u-v-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch11",
    name: "Practice Ru and U-Verbs - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of Ru-verbs and U-verbs.",
  },
  "genki_1_ch11_practice-ru-v-u-v-kana": {
    id: "genki_1_ch11_practice-ru-v-u-v-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch11",
    name: "Practice Ru and U-Verbs - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing Ru-verbs and U-verbs in Kana.",
  },
  "genki_1_ch11_irr-v-adv-misc": {
    id: "genki_1_ch11_irr-v-adv-misc",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch11",
    name: "Irr. Verbs, Adv, & Misc. Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions:
      "Learn irregular verbs, adverbs, and miscellaneous vocabulary from Chapter 11.",
  },
  "genki_1_ch11_practice-irr-v-adv-misc-readings": {
    id: "genki_1_ch11_practice-irr-v-adv-misc-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch11",
    name: "Practice Irr. V, Adv, & Misc. - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice the readings of irregular verbs, adverbs, and miscellaneous vocabulary.",
  },
  "genki_1_ch11_practice-irr-v-adv-misc-kana": {
    id: "genki_1_ch11_practice-irr-v-adv-misc-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch11",
    name: "Practice Irr. V, Adv, & Misc. - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice recognizing irregular verbs, adverbs, and miscellaneous vocabulary in Kana.",
  },
  genki_1_ch11_occupations: {
    id: "genki_1_ch11_occupations",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch11",
    name: "Occupations Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn vocabulary for occupations.",
  },
  "genki_1_ch11_practice-occupations-readings": {
    id: "genki_1_ch11_practice-occupations-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch11",
    name: "Practice Occupations - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of occupation vocabulary.",
  },
  "genki_1_ch11_practice-occupations-kana": {
    id: "genki_1_ch11_practice-occupations-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch11",
    name: "Practice Occupations - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing occupation vocabulary in Kana.",
  },
  "genki_1_ch11_all-vocab-test": {
    id: "genki_1_ch11_all-vocab-test",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch11",
    name: "All Vocab - Test",
    session_type: "vocab-test",
    ordered_vocab_keys: [],
    instructions: "Test your knowledge of all vocabulary from Chapter 11.",
  },

  // Chapter 12 Dynamic Modules
  "genki_1_ch12_vocab-list": {
    id: "genki_1_ch12_vocab-list",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch12",
    name: "Chapter 12 Vocab List",
    session_type: "vocab-list",
    ordered_vocab_keys: [],
    instructions: "Review the vocabulary for Chapter 12.",
  },
  "genki_1_ch12_nouns-1": {
    id: "genki_1_ch12_nouns-1",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch12",
    name: "Nouns 1 Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn the first set of nouns from Chapter 12.",
  },
  "genki_1_ch12_practice-nouns-1-readings": {
    id: "genki_1_ch12_practice-nouns-1-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch12",
    name: "Practice Nouns 1 - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of the first set of nouns.",
  },
  "genki_1_ch12_practice-nouns-1-kana": {
    id: "genki_1_ch12_practice-nouns-1-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch12",
    name: "Practice Nouns 1 - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing the first set of nouns in Kana.",
  },
  "genki_1_ch12_nouns-2": {
    id: "genki_1_ch12_nouns-2",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch12",
    name: "Nouns 2 Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn the second set of nouns from Chapter 12.",
  },
  "genki_1_ch12_practice-nouns-2-readings": {
    id: "genki_1_ch12_practice-nouns-2-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch12",
    name: "Practice Nouns 2 - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of the second set of nouns.",
  },
  "genki_1_ch12_practice-nouns-2-kana": {
    id: "genki_1_ch12_practice-nouns-2-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch12",
    name: "Practice Nouns 2 - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing the second set of nouns in Kana.",
  },
  genki_1_ch12_verbs: {
    id: "genki_1_ch12_verbs",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch12",
    name: "Verbs Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn verbs from Chapter 12.",
  },
  "genki_1_ch12_practice-verbs-readings": {
    id: "genki_1_ch12_practice-verbs-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch12",
    name: "Practice Verbs - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of verbs from Chapter 12.",
  },
  "genki_1_ch12_practice-verbs-kana": {
    id: "genki_1_ch12_practice-verbs-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch12",
    name: "Practice Verbs - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing verbs from Chapter 12 in Kana.",
  },
  "genki_1_ch12_adj-adv-misc": {
    id: "genki_1_ch12_adj-adv-misc",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch12",
    name: "Adj, Adv, & Misc. Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions:
      "Learn adjectives, adverbs, and miscellaneous vocabulary from Chapter 12.",
  },
  "genki_1_ch12_practice-adj-adv-misc-readings": {
    id: "genki_1_ch12_practice-adj-adv-misc-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch12",
    name: "Practice Adj, Adv, & Misc. - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice the readings of adjectives, adverbs, and miscellaneous vocabulary.",
  },
  "genki_1_ch12_practice-adj-adv-misc-kana": {
    id: "genki_1_ch12_practice-adj-adv-misc-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch12",
    name: "Practice Adj, Adv, & Misc. - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice recognizing adjectives, adverbs, and miscellaneous vocabulary in Kana.",
  },
  "genki_1_ch12_all-vocab-test": {
    id: "genki_1_ch12_all-vocab-test",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch12",
    name: "All Vocab - Test",
    session_type: "vocab-test",
    ordered_vocab_keys: [],
    instructions: "Test your knowledge of all vocabulary from Chapter 12.",
  },

  // Chapter 13 Dynamic Modules
  "genki_1_ch13_vocab-list": {
    id: "genki_1_ch13_vocab-list",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch13",
    name: "Chapter 13 Vocab List",
    session_type: "vocab-list",
    ordered_vocab_keys: [],
    instructions: "Review the vocabulary for Chapter 13.",
  },
  genki_1_ch13_nouns: {
    id: "genki_1_ch13_nouns",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch13",
    name: "Nouns Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn nouns from Chapter 13.",
  },
  "genki_1_ch13_practice-nouns-readings": {
    id: "genki_1_ch13_practice-nouns-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch13",
    name: "Practice Nouns - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of nouns from Chapter 13.",
  },
  "genki_1_ch13_practice-nouns-kana": {
    id: "genki_1_ch13_practice-nouns-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch13",
    name: "Practice Nouns - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing nouns from Chapter 13 in Kana.",
  },
  "genki_1_ch13_adj-and-verbs": {
    id: "genki_1_ch13_adj-and-verbs",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch13",
    name: "Adjectives & Verbs Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn adjectives and verbs from Chapter 13.",
  },
  "genki_1_ch13_practice-adj-and-verbs-readings": {
    id: "genki_1_ch13_practice-adj-and-verbs-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch13",
    name: "Practice Adj & Verbs - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice the readings of adjectives and verbs from Chapter 13.",
  },
  "genki_1_ch13_practice-adj-and-verbs-kana": {
    id: "genki_1_ch13_practice-adj-and-verbs-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch13",
    name: "Practice Adj & Verbs - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice recognizing adjectives and verbs from Chapter 13 in Kana.",
  },
  "genki_1_ch13_day-count-and-misc": {
    id: "genki_1_ch13_day-count-and-misc",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch13",
    name: "Day Count & Misc. Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn vocabulary for counting days and miscellaneous terms.",
  },
  "genki_1_ch13_practice-day-count-and-misc-readings": {
    id: "genki_1_ch13_practice-day-count-and-misc-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch13",
    name: "Practice Day Count & Misc. - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice the readings of day count and miscellaneous vocabulary.",
  },
  "genki_1_ch13_practice-day-count-and-misc-kana": {
    id: "genki_1_ch13_practice-day-count-and-misc-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch13",
    name: "Practice Day Count & Misc. - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice recognizing day count and miscellaneous vocabulary in Kana.",
  },
  "genki_1_ch13_all-vocab-test": {
    id: "genki_1_ch13_all-vocab-test",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch13",
    name: "All Vocab - Test",
    session_type: "vocab-test",
    ordered_vocab_keys: [],
    instructions: "Test your knowledge of all vocabulary from Chapter 13.",
  },

  // Chapter 14 Dynamic Modules
  "genki_1_ch14_vocab-list": {
    id: "genki_1_ch14_vocab-list",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch14",
    name: "Chapter 14 Vocab List",
    session_type: "vocab-list",
    ordered_vocab_keys: [],
    instructions: "Review the vocabulary for Chapter 14.",
  },
  "genki_1_ch14_nouns-1": {
    id: "genki_1_ch14_nouns-1",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch14",
    name: "Nouns 1 Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn the first set of nouns from Chapter 14.",
  },
  "genki_1_ch14_practice-nouns-1-readings": {
    id: "genki_1_ch14_practice-nouns-1-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch14",
    name: "Practice Nouns 1 - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of the first set of nouns.",
  },
  "genki_1_ch14_practice-nouns-1-kana": {
    id: "genki_1_ch14_practice-nouns-1-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch14",
    name: "Practice Nouns 1 - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing the first set of nouns in Kana.",
  },
  "genki_1_ch14_nouns-2": {
    id: "genki_1_ch14_nouns-2",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch14",
    name: "Nouns 2 Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn the second set of nouns from Chapter 14.",
  },
  "genki_1_ch14_practice-nouns-2-readings": {
    id: "genki_1_ch14_practice-nouns-2-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch14",
    name: "Practice Nouns 2 - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of the second set of nouns.",
  },
  "genki_1_ch14_practice-nouns-2-kana": {
    id: "genki_1_ch14_practice-nouns-2-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch14",
    name: "Practice Nouns 2 - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing the second set of nouns in Kana.",
  },
  "genki_1_ch14_adj-and-verbs": {
    id: "genki_1_ch14_adj-and-verbs",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch14",
    name: "Adjectives & Verbs Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn adjectives and verbs from Chapter 14.",
  },
  "genki_1_ch14_practice-adj-and-verbs-readings": {
    id: "genki_1_ch14_practice-adj-and-verbs-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch14",
    name: "Practice Adj & Verbs - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice the readings of adjectives and verbs from Chapter 14.",
  },
  "genki_1_ch14_practice-adj-and-verbs-kana": {
    id: "genki_1_ch14_practice-adj-and-verbs-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch14",
    name: "Practice Adj & Verbs - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice recognizing adjectives and verbs from Chapter 14 in Kana.",
  },
  "genki_1_ch14_counters-adv-misc": {
    id: "genki_1_ch14_counters-adv-misc",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch14",
    name: "Counters, Adv., & Misc. Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions:
      "Learn counters, adverbs, and miscellaneous vocabulary from Chapter 14.",
  },
  "genki_1_ch14_practice-counters-adv-misc-readings": {
    id: "genki_1_ch14_practice-counters-adv-misc-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch14",
    name: "Counters, Adv., & Misc. - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice the readings of counters, adverbs, and miscellaneous vocabulary.",
  },
  "genki_1_ch14_practice-counters-adv-misc-kana": {
    id: "genki_1_ch14_practice-counters-adv-misc-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch14",
    name: "Counters, Adv., & Misc. - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice recognizing counters, adverbs, and miscellaneous vocabulary in Kana.",
  },
  "genki_1_ch14_all-vocab-test": {
    id: "genki_1_ch14_all-vocab-test",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch14",
    name: "All Vocab - Test",
    session_type: "vocab-test",
    ordered_vocab_keys: [],
    instructions: "Test your knowledge of all vocabulary from Chapter 14.",
  },

  // Chapter 15 Dynamic Modules
  "genki_1_ch15_vocab-list": {
    id: "genki_1_ch15_vocab-list",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch15",
    name: "Chapter 15 Vocab List",
    session_type: "vocab-list",
    ordered_vocab_keys: [],
    instructions: "Review the vocabulary for Chapter 15.",
  },
  "genki_1_ch15_nouns-1": {
    id: "genki_1_ch15_nouns-1",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch15",
    name: "Nouns 1 Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn the first set of nouns from Chapter 15.",
  },
  "genki_1_ch15_practice-nouns-1-readings": {
    id: "genki_1_ch15_practice-nouns-1-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch15",
    name: "Practice Nouns 1 - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of the first set of nouns.",
  },
  "genki_1_ch15_practice-nouns-1-kana": {
    id: "genki_1_ch15_practice-nouns-1-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch15",
    name: "Practice Nouns 1 - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing the first set of nouns in Kana.",
  },
  "genki_1_ch15_nouns-2": {
    id: "genki_1_ch15_nouns-2",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch15",
    name: "Nouns 2 Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn the second set of nouns from Chapter 15.",
  },
  "genki_1_ch15_practice-nouns-2-readings": {
    id: "genki_1_ch15_practice-nouns-2-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch15",
    name: "Practice Nouns 2 - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of the second set of nouns.",
  },
  "genki_1_ch15_practice-nouns-2-kana": {
    id: "genki_1_ch15_practice-nouns-2-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch15",
    name: "Practice Nouns 2 - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing the second set of nouns in Kana.",
  },
  "genki_1_ch15_godan-and-ichidan-v": {
    id: "genki_1_ch15_godan-and-ichidan-v",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch15",
    name: "Ichidan & Godan Verbs Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn Ichidan and Godan verbs from Chapter 15.",
  },
  "genki_1_ch15_practice-godan-and-ichidan-v-readings": {
    id: "genki_1_ch15_practice-godan-and-ichidan-v-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch15",
    name: "Practice Ichidan & Godan V. - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of Ichidan and Godan verbs.",
  },
  "genki_1_ch15_practice-godan-and-ichidan-v-kana": {
    id: "genki_1_ch15_practice-godan-and-ichidan-v-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch15",
    name: "Practice Ichidan & Godan V. - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing Ichidan and Godan verbs in Kana.",
  },
  "genki_1_ch15_irr-v-adv-misc": {
    id: "genki_1_ch15_irr-v-adv-misc",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch15",
    name: "Irreg Verbs, Adv., & Misc. Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions:
      "Learn irregular verbs, adverbs, and miscellaneous vocabulary from Chapter 15.",
  },
  "genki_1_ch15_practice-irr-v-adv-misc-readings": {
    id: "genki_1_ch15_practice-irr-v-adv-misc-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch15",
    name: "Irreg Verbs, Adv., & Misc. - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice the readings of irregular verbs, adverbs, and miscellaneous vocabulary.",
  },
  "genki_1_ch15_practice-irr-v-adv-misc-kana": {
    id: "genki_1_ch15_practice-irr-v-adv-misc-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch15",
    name: "Irreg Verbs, Adv., & Misc. - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice recognizing irregular verbs, adverbs, and miscellaneous vocabulary in Kana.",
  },
  "genki_1_ch15_all-vocab-test": {
    id: "genki_1_ch15_all-vocab-test",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch15",
    name: "All Vocab - Test",
    session_type: "vocab-test",
    ordered_vocab_keys: [],
    instructions: "Test your knowledge of all vocabulary from Chapter 15.",
  },

  // Chapter 16 Dynamic Modules
  "genki_1_ch16_vocab-list": {
    id: "genki_1_ch16_vocab-list",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch16",
    name: "Chapter 16 Vocab List",
    session_type: "vocab-list",
    ordered_vocab_keys: [],
    instructions: "Review the vocabulary for Chapter 16.",
  },
  genki_1_ch16_nouns: {
    id: "genki_1_ch16_nouns",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch16",
    name: "Nouns Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn nouns from Chapter 16.",
  },
  "genki_1_ch16_practice-nouns-readings": {
    id: "genki_1_ch16_practice-nouns-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch16",
    name: "Practice Nouns - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of nouns from Chapter 16.",
  },
  "genki_1_ch16_practice-nouns-kana": {
    id: "genki_1_ch16_practice-nouns-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch16",
    name: "Practice Nouns - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing nouns from Chapter 16 in Kana.",
  },
  "genki_1_ch16_adj-and-u-v": {
    id: "genki_1_ch16_adj-and-u-v",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch16",
    name: "Adjectives & U-Verbs Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn adjectives and U-verbs from Chapter 16.",
  },
  "genki_1_ch16_practice-adj-and-u-v-readings": {
    id: "genki_1_ch16_practice-adj-and-u-v-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch16",
    name: "Practice Adj. & U-V - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of adjectives and U-verbs.",
  },
  "genki_1_ch16_practice-adj-and-u-v-kana": {
    id: "genki_1_ch16_practice-adj-and-u-v-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch16",
    name: "Practice Adj. & U-V - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing adjectives and U-verbs in Kana.",
  },
  "genki_1_ch16_ru-and-irr-v": {
    id: "genki_1_ch16_ru-and-irr-v",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch16",
    name: "Ru-Verbs & Irregular Verbs Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn Ru-verbs and irregular verbs from Chapter 16.",
  },
  "genki_1_ch16_practice-ru-and-irr-v-readings": {
    id: "genki_1_ch16_practice-ru-and-irr-v-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch16",
    name: "Practice Ru-V & Irreg V - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of Ru-verbs and irregular verbs.",
  },
  "genki_1_ch16_practice-ru-and-irr-v-kana": {
    id: "genki_1_ch16_practice-ru-and-irr-v-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch16",
    name: "Practice Ru-V & Irreg V - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing Ru-verbs and irregular verbs in Kana.",
  },
  "genki_1_ch16_adv-and-misc": {
    id: "genki_1_ch16_adv-and-misc",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch16",
    name: "Adverbs & Misc. Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn adverbs and miscellaneous vocabulary from Chapter 16.",
  },
  "genki_1_ch16_practice-adv-and-misc-readings": {
    id: "genki_1_ch16_practice-adv-and-misc-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch16",
    name: "Practice Adv & Misc. - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice the readings of adverbs and miscellaneous vocabulary.",
  },
  "genki_1_ch16_practice-adv-and-misc-kana": {
    id: "genki_1_ch16_practice-adv-and-misc-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch16",
    name: "Practice Adv & Misc. - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice recognizing adverbs and miscellaneous vocabulary in Kana.",
  },
  "genki_1_ch16_all-vocab-test": {
    id: "genki_1_ch16_all-vocab-test",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch16",
    name: "All Vocab - Test",
    session_type: "vocab-test",
    ordered_vocab_keys: [],
    instructions: "Test your knowledge of all vocabulary from Chapter 16.",
  },

  // Chapter 17 Dynamic Modules
  "genki_1_ch17_vocab-list": {
    id: "genki_1_ch17_vocab-list",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch17",
    name: "Chapter 17 Vocab List",
    session_type: "vocab-list",
    ordered_vocab_keys: [],
    instructions: "Review the vocabulary for Chapter 17.",
  },
  "genki_1_ch17_nouns-1": {
    id: "genki_1_ch17_nouns-1",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch17",
    name: "Nouns 1 Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn the first set of nouns from Chapter 17.",
  },
  "genki_1_ch17_practice-nouns-1-readings": {
    id: "genki_1_ch17_practice-nouns-1-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch17",
    name: "Practice Nouns 1 - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of the first set of nouns.",
  },
  "genki_1_ch17_practice-nouns-1-kana": {
    id: "genki_1_ch17_practice-nouns-1-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch17",
    name: "Practice Nouns 1 - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing the first set of nouns in Kana.",
  },
  "genki_1_ch17_nouns-2": {
    id: "genki_1_ch17_nouns-2",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch17",
    name: "Nouns 2 Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn the second set of nouns from Chapter 17.",
  },
  "genki_1_ch17_practice-nouns-2-readings": {
    id: "genki_1_ch17_practice-nouns-2-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch17",
    name: "Practice Nouns 2 - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of the second set of nouns.",
  },
  "genki_1_ch17_practice-nouns-2-kana": {
    id: "genki_1_ch17_practice-nouns-2-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch17",
    name: "Practice Nouns 2 - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing the second set of nouns in Kana.",
  },
  genki_1_ch17_verbs: {
    id: "genki_1_ch17_verbs",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch17",
    name: "Verbs Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn verbs from Chapter 17.",
  },
  "genki_1_ch17_practice-verbs-readings": {
    id: "genki_1_ch17_practice-verbs-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch17",
    name: "Practice Verbs - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of verbs from Chapter 17.",
  },
  "genki_1_ch17_practice-verbs-kana": {
    id: "genki_1_ch17_practice-verbs-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch17",
    name: "Practice Verbs - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing verbs from Chapter 17 in Kana.",
  },
  "genki_1_ch17_adj-adv-misc": {
    id: "genki_1_ch17_adj-adv-misc",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch17",
    name: "Adj, Adv, & Misc. Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions:
      "Learn adjectives, adverbs, and miscellaneous vocabulary from Chapter 17.",
  },
  "genki_1_ch17_practice-adj-adv-misc-readings": {
    id: "genki_1_ch17_practice-adj-adv-misc-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch17",
    name: "Practice Adj, Adv, & Misc. - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice the readings of adjectives, adverbs, and miscellaneous vocabulary.",
  },
  "genki_1_ch17_practice-adj-adv-misc-kana": {
    id: "genki_1_ch17_practice-adj-adv-misc-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch17",
    name: "Practice Adj, Adv, & Misc. - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice recognizing adjectives, adverbs, and miscellaneous vocabulary in Kana.",
  },
  "genki_1_ch17_all-vocab-test": {
    id: "genki_1_ch17_all-vocab-test",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch17",
    name: "All Vocab - Test",
    session_type: "vocab-test",
    ordered_vocab_keys: [],
    instructions: "Test your knowledge of all vocabulary from Chapter 17.",
  },
  "sentence-practice-sou-desu-heresay": {
    id: "sentence-practice-sou-desu-heresay",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch17",
    name: "～そうです - Heresay Practice",
    session_type: "sentence-practice",
    ordered_vocab_keys: ["genki_1/sentence-practice-sou-desu-heresay"],
    instructions: "Practice using ～そうです to report what you've heard.",
  },
  "sentence-practice-tte": {
    id: "sentence-practice-tte",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch17",
    name: "～って Practice",
    session_type: "sentence-practice",
    ordered_vocab_keys: ["genki_1/sentence-practice-tte"],
    instructions: "Practice using the casual particle ～って.",
  },
  "sentence-practice-tara": {
    id: "sentence-practice-tara",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch17",
    name: "～たら Practice",
    session_type: "sentence-practice",
    ordered_vocab_keys: ["genki_1/sentence-practice-tara"],
    instructions: "Practice using the conditional form ～たら.",
  },

  // Chapter 18 Dynamic Modules
  "genki_1_ch18_vocab-list": {
    id: "genki_1_ch18_vocab-list",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch18",
    name: "Chapter 18 Vocab List",
    session_type: "vocab-list",
    ordered_vocab_keys: [],
    instructions: "Review the vocabulary for Chapter 18.",
  },
  "genki_1_ch18_nouns-1": {
    id: "genki_1_ch18_nouns-1",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch18",
    name: "Nouns 1 Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn the first set of nouns from Chapter 18.",
  },
  "genki_1_ch18_practice-nouns-1-readings": {
    id: "genki_1_ch18_practice-nouns-1-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch18",
    name: "Practice Nouns 1 - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of the first set of nouns.",
  },
  "genki_1_ch18_practice-nouns-1-kana": {
    id: "genki_1_ch18_practice-nouns-1-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch18",
    name: "Practice Nouns 1 - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing the first set of nouns in Kana.",
  },
  "genki_1_ch18_nouns-2-and-adjectives": {
    id: "genki_1_ch18_nouns-2-and-adjectives",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch18",
    name: "Nouns 2 & Adjectives Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions:
      "Learn the second set of nouns and adjectives from Chapter 18.",
  },
  "genki_1_ch18_practice-nouns-2-and-adjectives-readings": {
    id: "genki_1_ch18_practice-nouns-2-and-adjectives-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch18",
    name: "Practice Nouns 2 & Adj - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice the readings of the second set of nouns and adjectives.",
  },
  "genki_1_ch18_practice-nouns-2-and-adjectives-kana": {
    id: "genki_1_ch18_practice-nouns-2-and-adjectives-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch18",
    name: "Practice Nouns 2 & Adj - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice recognizing the second set of nouns and adjectives in Kana.",
  },
  "genki_1_ch18_u-and-irr-verbs": {
    id: "genki_1_ch18_u-and-irr-verbs",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch18",
    name: "U & Irr Verbs Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn U-verbs and irregular verbs from Chapter 18.",
  },
  "genki_1_ch18_practice-u-and-irr-verbs-readings": {
    id: "genki_1_ch18_practice-u-and-irr-verbs-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch18",
    name: "Practice U & Irr Verbs - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of U-verbs and irregular verbs.",
  },
  "genki_1_ch18_practice-u-and-irr-verbs-kana": {
    id: "genki_1_ch18_practice-u-and-irr-verbs-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch18",
    name: "Practice U & Irr Verbs - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing U-verbs and irregular verbs in Kana.",
  },
  "genki_1_ch18_ru-verbs-and-misc": {
    id: "genki_1_ch18_ru-verbs-and-misc",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch18",
    name: "Ru Verbs & Misc. Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions:
      "Learn Ru-verbs and miscellaneous vocabulary from Chapter 18.",
  },
  "genki_1_ch18_practice-ru-verbs-and-misc-readings": {
    id: "genki_1_ch18_practice-ru-verbs-and-misc-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch18",
    name: "Practice Ru Verbs & Misc. - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice the readings of Ru-verbs and miscellaneous vocabulary.",
  },
  "genki_1_ch18_practice-ru-verbs-and-misc-kana": {
    id: "genki_1_ch18_practice-ru-verbs-and-misc-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch18",
    name: "Practice Ru Verbs & Misc. - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice recognizing Ru-verbs and miscellaneous vocabulary in Kana.",
  },
  "genki_1_ch18_all-vocab-test": {
    id: "genki_1_ch18_all-vocab-test",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch18",
    name: "All Vocab - Test",
    session_type: "vocab-test",
    ordered_vocab_keys: [],
    instructions: "Test your knowledge of all vocabulary from Chapter 18.",
  },

  // Chapter 19 Dynamic Modules
  "genki_1_ch19_vocab-list": {
    id: "genki_1_ch19_vocab-list",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch19",
    name: "Chapter 19 Vocab List",
    session_type: "vocab-list",
    ordered_vocab_keys: [],
    instructions: "Review the vocabulary for Chapter 19.",
  },
  "genki_1_ch19_chapter-13-18-bonus-vocab": {
    id: "genki_1_ch19_chapter-13-18-bonus-vocab",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch19",
    name: "Ch. 13-18 Bonus Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Review bonus vocabulary from Chapters 13-18.",
  },
  "genki_1_ch19_chapter-13-18-nouns": {
    id: "genki_1_ch19_chapter-13-18-nouns",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch19",
    name: "Ch. 13-18 Nouns - Test",
    session_type: "vocab-test",
    ordered_vocab_keys: [],
    instructions: "Test your knowledge of nouns from Chapters 13-18.",
  },
  "genki_1_ch19_chapter-13-18-adjectives": {
    id: "genki_1_ch19_chapter-13-18-adjectives",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch19",
    name: "Ch. 13-18 Adjectives - Test",
    session_type: "vocab-test",
    ordered_vocab_keys: [],
    instructions: "Test your knowledge of adjectives from Chapters 13-18.",
  },
  "genki_1_ch19_chapter-13-18-verbs": {
    id: "genki_1_ch19_chapter-13-18-verbs",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch19",
    name: "Ch. 13-18 Verbs - Test",
    session_type: "vocab-test",
    ordered_vocab_keys: [],
    instructions: "Test your knowledge of verbs from Chapters 13-18.",
  },
  "genki_1_ch19_chapter-13-18-adv-misc": {
    id: "genki_1_ch19_chapter-13-18-adv-misc",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch19",
    name: "Ch. 13-18 Adv., Misc. - Test",
    session_type: "vocab-test",
    ordered_vocab_keys: [],
    instructions:
      "Test your knowledge of adverbs and miscellaneous vocabulary from Chapters 13-18.",
  },
  "genki_1_ch19_nouns-and-adj": {
    id: "genki_1_ch19_nouns-and-adj",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch19",
    name: "Nouns & Adj. Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn nouns and adjectives from Chapter 19.",
  },
  "genki_1_ch19_practice-nouns-and-adj-readings": {
    id: "genki_1_ch19_practice-nouns-and-adj-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch19",
    name: "Practice Nouns & Adj. - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice the readings of nouns and adjectives from Chapter 19.",
  },
  "genki_1_ch19_practice-nouns-and-adj-kana": {
    id: "genki_1_ch19_practice-nouns-and-adj-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch19",
    name: "Practice Nouns & Adj. - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice recognizing nouns and adjectives from Chapter 19 in Kana.",
  },
  "genki_1_ch19_u-ru-verbs": {
    id: "genki_1_ch19_u-ru-verbs",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch19",
    name: "U + Ru Verbs Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn U and Ru verbs from Chapter 19.",
  },
  "genki_1_ch19_practice-u-ru-verbs-readings": {
    id: "genki_1_ch19_practice-u-ru-verbs-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch19",
    name: "Practice U + Ru Verbs - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of U and Ru verbs from Chapter 19.",
  },
  "genki_1_ch19_practice-u-ru-verbs-kana": {
    id: "genki_1_ch19_practice-u-ru-verbs-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch19",
    name: "Practice U + Ru Verbs - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice recognizing U and Ru verbs from Chapter 19 in Kana.",
  },
  "genki_1_ch19_irr-v-adv-misc": {
    id: "genki_1_ch19_irr-v-adv-misc",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch19",
    name: "Irr. Verbs, Adv., & Misc. Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions:
      "Learn irregular verbs, adverbs, and miscellaneous vocabulary from Chapter 19.",
  },
  "genki_1_ch19_practice-irr-v-adv-misc-readings": {
    id: "genki_1_ch19_practice-irr-v-adv-misc-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch19",
    name: "Practice Irr. Verbs, Adv., & Misc. - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice the readings of irregular verbs, adverbs, and miscellaneous vocabulary.",
  },
  "genki_1_ch19_practice-irr-v-adv-misc-kana": {
    id: "genki_1_ch19_practice-irr-v-adv-misc-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch19",
    name: "Practice Irr. Verbs, Adv., & Misc. - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice recognizing irregular verbs, adverbs, and miscellaneous vocabulary in Kana.",
  },
  "genki_1_ch19_all-vocab-test": {
    id: "genki_1_ch19_all-vocab-test",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch19",
    name: "Ch. 19 All Vocab - Test",
    session_type: "vocab-test",
    ordered_vocab_keys: [],
    instructions: "Test your knowledge of all vocabulary from Chapter 19.",
  },
  "sentence-practice-honorific-verbs": {
    id: "sentence-practice-honorific-verbs",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch19",
    name: "Honorific Verbs Sentence Practice",
    session_type: "sentence-practice",
    ordered_vocab_keys: ["genki_1/sentence-practice-honorific-verbs"],
    instructions: "Practice using honorific verbs in sentences.",
  },

  // Chapter 20 Dynamic Modules
  "genki_1_ch20_vocab-list": {
    id: "genki_1_ch20_vocab-list",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch20",
    name: "Chapter 20 Vocab List",
    session_type: "vocab-list",
    ordered_vocab_keys: [],
    instructions: "Review the vocabulary for Chapter 20.",
  },
  genki_1_ch20_nouns: {
    id: "genki_1_ch20_nouns",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch20",
    name: "Nouns Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn nouns from Chapter 20.",
  },
  "genki_1_ch20_practice-nouns-readings": {
    id: "genki_1_ch20_practice-nouns-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch20",
    name: "Practice Nouns - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of nouns from Chapter 20.",
  },
  "genki_1_ch20_practice-nouns-kana": {
    id: "genki_1_ch20_practice-nouns-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch20",
    name: "Practice Nouns - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing nouns from Chapter 20 in Kana.",
  },
  "genki_1_ch20_adj-u-v": {
    id: "genki_1_ch20_adj-u-v",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch20",
    name: "Adjectives & う-Verbs Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions: "Learn adjectives and う-verbs from Chapter 20.",
  },
  "genki_1_ch20_practice-adj-u-v-readings": {
    id: "genki_1_ch20_practice-adj-u-v-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch20",
    name: "Practice Adjectives & う-Verbs - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice the readings of adjectives and う-verbs.",
  },
  "genki_1_ch20_practice-adj-u-v-kana": {
    id: "genki_1_ch20_practice-adj-u-v-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch20",
    name: "Practice Adjectives & う-Verbs - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions: "Practice recognizing adjectives and う-verbs in Kana.",
  },
  "genki_1_ch20_ru-v-irr-v-adv-misc": {
    id: "genki_1_ch20_ru-v-irr-v-adv-misc",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch20",
    name: "る-Verbs, Irr. V., Adv., & Misc. Vocab",
    session_type: "vocab-sublist",
    ordered_vocab_keys: [],
    instructions:
      "Learn る-verbs, irregular verbs, adverbs, and miscellaneous vocabulary from Chapter 20.",
  },
  "genki_1_ch20_practice-ru-v-irr-v-adv-misc-readings": {
    id: "genki_1_ch20_practice-ru-v-irr-v-adv-misc-readings",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch20",
    name: "Practice る-Verbs, Irr. V., Adv., & Misc. - Readings",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice the readings of る-verbs, irregular verbs, adverbs, and miscellaneous vocabulary.",
  },
  "genki_1_ch20_practice-ru-v-irr-v-adv-misc-kana": {
    id: "genki_1_ch20_practice-ru-v-irr-v-adv-misc-kana",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch20",
    name: "Practice る-Verbs, Irr. V., Adv., & Misc. - Kana",
    session_type: "vocab-practice",
    ordered_vocab_keys: [],
    instructions:
      "Practice recognizing る-verbs, irregular verbs, adverbs, and miscellaneous vocabulary in Kana.",
  },
  "genki_1_ch20_all-vocab-test": {
    id: "genki_1_ch20_all-vocab-test",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch20",
    name: "All Vocab - Test",
    session_type: "vocab-test",
    ordered_vocab_keys: [],
    instructions: "Test your knowledge of all vocabulary from Chapter 20.",
  },
  "sentence-practice-extra-modest-expressions": {
    id: "sentence-practice-extra-modest-expressions",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch20",
    name: "Extra Modest Expressions Sentence Practice",
    session_type: "sentence-practice",
    ordered_vocab_keys: ["genki_1/sentence-practice-extra-modest-expressions"],
    instructions: "Practice using extra modest expressions in sentences.",
  },
  "sentence-practice-humble-expressions": {
    id: "sentence-practice-humble-expressions",
    textbook_id: "genki_1",
    chapter_id: "genki_1_ch20",
    name: "Humble Expressions Sentence Practice",
    session_type: "sentence-practice",
    ordered_vocab_keys: ["genki_1/sentence-practice-humble-expressions"],
    instructions: "Practice using humble expressions in sentences.",
  },
}
